...
# interval: How often (in seconds) this agent should initiate a message. Default is 150s (2 minutes 30 seconds) for Agent C.
def send_gemini_message_periodically(interval=150):
    while True:
        ...
        try:
            ...
            # Agent C will target either Agent A or Agent B.
            target_agent = random.choice(['agent_a', 'agent_b'])
...
while True:
    try:
        ...
        # Pass the interval (150s for Agent C) as an argument.
        sender_thread = threading.Thread(target=send_gemini_message_periodically, args=(150,), daemon=True)
        sender_thread.start()
