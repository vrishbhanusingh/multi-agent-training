...
AGENT_NAME = os.getenv("AGENT_NAME", "agent_c")  # Adjusted default agent name
...
if USE_VERTEX:
    # Vertex AI logic - assuming correct or handled separately
    pass
else:
    if not API_KEY:
...
def ask_gemini(query: str, model_name: str = MODEL, use_memory: bool = False) -> str:  # Renamed model parameter
    """
    Send a query to Gemini 2.5 LLM and return the response text.
    ...
    response = model_instance.generate_content(contents=query)  # Use model_instance
